...
D uint32[[2]] duplicate(D uint32[[2]] rel, uint numDups) {
    ...
    for (uint r = 0; r < numRows; r+=1) {
        dupped[r * numDups : (r + 1) * numDups,:] = rel[:,:];
    }
    return dupped;
}
...
template <domain D : shared3p>
D uint32[[2]] obliviousJoin(D uint32[[2]] leftRel, uint[[1]] leftJoinCols,
...
D uint32[[2]] flagJoin(D uint32[[2]] eqFlags, D uint32[[2]] left, uint leftJoinCol, D uint32[[2]] right,
    ...
    D uint32[[2]] res = joinFromEqFlags(eqFlags, leftStretched, leftJoinColIdx, rightDupped, rightJoinColIdx);
    return project(res, colsToKeep);
}

template <domain D : shared3p>
D uint32[[2]] index(D uint32[[2]] values, D uint32[[1]] flags) {
...
template <domain D : shared3p>
D uint32[[2]] pad(D uint32[[2]] values, uint numPads) {
    uint nrows = shape(values)[0];
    uint ncols = shape(values)[1];
...
template <domain D : shared3p>
void process_rel(string prefix, D uint32[[2]] input, D uint32[[1]] encoded, uint numLookups) {
//    uint nrows = shape(input)[0];
//    uint ncols = shape(input)[1];
    pd_shared3p uint32[[2]] indexed = index(input, encoded);

    pd_shared3p uint32[[2]] padded = pad(indexed, numLookups);
...
void main() {
    print("Running hybrid_ssn shark_join_1");
    string ds = "DS1";
    tdbOpenConnection(ds);
    uint numLookups = (uint) declassify(readFromDb("DS1", "num_lookups")[0 , 0]);
    // process in1
    pd_shared3p uint32[[1]] in1_encoded = readFromDb("DS1", "in1_encoded")[ : , 0];
    pd_shared3p uint32[[2]] in1_shuffled = readFromDb("DS1", "in1_shuffled");
    process_rel("in1", in1_shuffled, in1_encoded, numLookups);
    // process in2
    pd_shared3p uint32[[1]] in2_encoded = readFromDb("DS1", "in2_encoded")[ : , 0];
    pd_shared3p uint32[[2]] in2_shuffled = readFromDb("DS1", "in2_shuffled");
    process_rel("in2", in2_shuffled, in2_encoded, numLookups);
    // done
