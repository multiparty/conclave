...
void main() {
    ...
    print("Running hybrid_ssn shuffle_inputs_1");
    pd_shared3p uint32 [[2]] in1 = readFromDb("DS1", "govreg");
    pd_shared3p uint32 [[2]] in2 = cat(readFromDb("DS1", "company0"), readFromDb("DS1", "company1"));
    pd_shared3p uint32 [[2]] in1_shuffled = shuffleRows(in1);
    pd_shared3p uint32 [[2]] in2_shuffled = shuffleRows(in2);
    persist(ds, "in1_shuffled", in1_shuffled);
    persist(ds, "in2_shuffled", in2_shuffled);
    pd_shared3p uint32 [[2]] in1_keys = project(in1_shuffled, (uint){0});
    pd_shared3p uint32 [[2]] in2_keys = project(in2_shuffled, (uint){0});
    print("Will publish in1_keys size: ");
    print(shape(in1_keys)[0]);
    publish("in1_keys", declassify(in1_keys));
    print("Will publish in2_keys size: ");
    print(shape(in2_keys)[0]);
    publish("in2_keys", declassify(in2_keys));
