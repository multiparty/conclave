...
def _aggregate_count_dict(rel, group_by_idx, over_idx, aggregator):
    acc = {}
    for row in rel:
        key = row[group_by_idx]
        if key not in acc:
...
def aggregate_count(rel, group_by_idx, over_idx, aggregator):
    return [[key, value] for key, value in _aggregate_count_dict(rel, group_by_idx, over_idx, aggregator).items()]

def compute_shark_flags(lookups, indexes_and_flags):
    lookups_counts = _aggregate_count_dict(lookups, 0, -1, "")
    ...
    smarties = deque(in_order[:len(lookups_counts)]) 
    dummies = deque(in_order[len(lookups_counts):])
    res = []
    for smarty in smarties:
        res.append(smarty)
        num_dummies = lookups_counts[smarty[1]] - 1
        for _ in range(num_dummies):
            res.append(dummies.pop())
    return res

def encode_as_flags(lookup, rel_size):
    res = [[0] for _ in range(rel_size)]
    for idx in lookup:
        res[idx[0]] = [1]
    return res

def get_original_ordering(rel):
    projected = project_indeces(rel)
    reordered = sort_by(projected, 1)
    return reordered
